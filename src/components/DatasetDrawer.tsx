...
export const DatasetDrawer: React.FC<DatasetDrawerProps> = ({
  ...
  const processFile = (file: File) => {
    ...
    // Guard 1: Size limits (max 2MB) to prevent browser thread freeze
    const MAX_FILE_SIZE = 2 * 1024 * 1024;
    if (file.size > MAX_FILE_SIZE) {
      setUploadError(`File is too large (${(file.size / (1024 * 1024)).toFixed(2)}MB). Maximum allowed size is 2MB to keep visualization performance responsive.`);
      return;
    }

    // Guard 2: Extension/Mime-type validation
    const fileName = file.name;
    const lowerName = fileName.toLowerCase();
    if (!lowerName.endsWith('.json') && !lowerName.endsWith('.csv') && !lowerName.endsWith('.tsv') && !lowerName.endsWith('.txt')) {
      setUploadError('Unsupported file type. Please upload a structured .csv, .json, .tsv, or .txt file.');
      return;
    }
    ...
    reader.onerror = () => {
      setUploadError('Failed to read file from disk. Please verify the file is not locked or corrupted.');
    };
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (records.length === 0) {
          setUploadError('The uploaded dataset contains zero valid data rows or fields.');
          return;
        }
        ...
        setUploadError(`Parsing error: ${err.message || err}`);
      }
    };
  ...
  const renderContent = () => (
    <div className={`flex flex-col gap-4 h-full bg-white dark:bg-[#171717] transition-colors ${isInline ? 'w-full p-4 sm:p-5 overflow-y-auto' : ''}`}>
      ...
      {uploadError && (
        <div className="p-3 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 rounded-md text-xs text-red-600 dark:text-red-400 leading-normal flex items-start gap-2 shrink-0">
          <span className="font-semibold shrink-0">Error:</span>
          <span className="flex-1 font-sans">{uploadError}</span>
          <button 
            onClick={() => setUploadError(null)}
            className="text-red-400 hover:text-red-600 dark:hover:text-red-300 font-mono text-[10px] uppercase font-bold tracking-tight shrink-0 px-1 hover:bg-red-100 dark:hover:bg-red-900/30 rounded transition-colors"
          >
            Dismiss
          </button>
        </div>
